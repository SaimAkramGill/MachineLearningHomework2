...
class MLPClassifierOwn():
    ...
    def softmax(z: List[Scalar]) -> List[Scalar]:
        ...
        """
        # Compute exp(z_i) for each z_i
        exps = [z_i.exp() for z_i in z]
        # Compute sum of all exp(z_i)
        sum_exp = sum(exps)
        # Compute softmax: exp(z_i) / sum(exp(z_j))
        return [exp_i / sum_exp for exp_i in exps]
    ...
    def sigmoid(z: Scalar) -> Scalar:
        ...
        return Scalar(1.0) / (Scalar(1.0) + (-z).exp())
    ...
    def l2_regularization_term(self) -> Scalar:
        ...
        sum_of_squares = sum([param * param for param in params])
        
        # Apply the L2 regularization formula: (alpha / (2 * batch_size)) * sum(w^2)
        return (Scalar(self.alpha) / Scalar(2.0 * self.batch_size)) * sum_of_squares
